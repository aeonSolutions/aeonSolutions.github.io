// KiCad Web Viewer library, vanilla Javascript, bundles SVGPanZoom.js
// v0.02 May/2021
...
var KiCadWebView = {
  ...
  // (viewbox set by KiCad is buggy)
  // TODO: faster using .pathSegList if available? (not Chrome48+)
  viewbox: function(svg, aspect) {
    var min = [Infinity, Infinity], max = [-Infinity, -Infinity];
    var bignum = new RegExp('[vhl] ?-?(\\d){4,}', 'i'); // IE space-separates commands
    var split = new RegExp('[a-z][^a-z]*', 'ig');
    var els = svg.getElementsByTagName("path");
    for (var k = 0; k < els.length; k++) {
      var d = els[k].getAttribute('d');
      if (!bignum.test(d)) continue;  // skip paths without 4+ digit v/h/l command?
      var segs = d.match(split);
      var x = 0, y = 0;
      for (var i = 0; i < segs.length; i++) {
        var rel = (segs[i][0] == segs[i][0].toLowerCase());
        switch(segs[i][0]) {
          case 'M':
          case 'm':
          case 'L':
          case 'l':
            var s = segs[i].match(/(-?[0-9\\.]+) ?(-?[0-9\\.]+)/);
            x = (rel ? x : 0) + parseFloat(s[1]);
            y = (rel ? y : 0) + parseFloat(s[2]);
            break;
          case 'H':
          case 'h':
            var s = segs[i].match(/(-?[0-9\\.]+)/);
            x = (rel ? x : 0) + parseFloat(s[1]);
            break;
          case 'V':
          case 'v':
            var s = segs[i].match(/(-?[0-9\\.]+)/);
            y = (rel ? y : 0) + parseFloat(s[1]);
            break;
        };
        if (min[0] > x) min[0] = x;
        if (min[1] > y) min[1] = y;
    ...
    // center svg within new viewBox that matches 'aspect' ratio
    var w = max[0]+400-min[0];
    var h = max[1]+400-min[1];
    if (w/h < aspect) {
      var w2 = h*aspect;
      var x2 = (min[0]-200)-(w2-w)/2;
      return {x:x2, y:min[1]-200, width:w2, height:h}
    } else {
      var h2 = w/aspect;
      var y2 = (min[1]-200)-(h2-h)/2;
      return {x:min[0]-200, y:y2, width:w, height:h2}
    }
  },
  
  
  // Initialise SVG, shared between pcb() and schematic()
  initialise: function(svg, pcb, aspect) {
    var wrap, find, form, ui, panzoom;
    if (!(svg instanceof Element)) throw Error("KiCadWebView element not found");
    aspect = aspect || 600/412;
    // Wrap element
    ...
    if (pcb) {
      ...
      ui.className = 'kicadui';
      wrap.appendChild(ui)
    } else ui = null;
    form = document.createElement('form');
    form.className = 'kicadfind';
    form.onsubmit = this.findtext;
    ...
    find.addEventListener("blur", function(e){
      KiCadWebView.findbox(wrap, false)
    });
    var box = this.viewbox(svg, aspect);    
    svg.style.paddingBottom = (100/aspect).toFixed(2)+'%';
    
    // Register with panzoom library
    panzoom = new SVGPanZoom(svg, {initialViewBox:box, animationTime:0, zoom:{minZoom:1,maxZoom:10,callback:function(mul){
      if (mul <= 1.0) panzoom.reset() // re-center if zoom out to maximum
    }}})
    svg.setAttribute('preserveAspectRatio', "xMidYMin slice");
    wrap._panzoom = panzoom;
    // Event handlers for keyboard shortcuts
    wrap.addEventListener("mouseenter", function(e){
      this.focus();
    });
    wrap.addEventListener("keydown", this.keydown);
    // Log all mouse movements so we can do F4 keypress to re-center SVG :/
  ...
  // Lookup SVG stylesheet
  // IE9-11 missing local svg .sheet, so search in document.styleSheets[]
  getsheet: function(svg) {
    if (svg.firstChild.sheet) return svg.firstChild.sheet;
    var sheets = document.styleSheets;
    for (var i = sheets.length-1; i > 0; i--) {
      if (sheets[i].ownerNode === svg.firstChild) return sheets[i];
    }
    return {cssRules:[]};
  },
  
  
  // Rewrite all classes/ids with unique postfix (multiple inline SVGs share namespace)
  count: 1,
  rewrite: function(svg) {
    var sheet = this.getsheet(svg);
    var rules = sheet.cssRules;
    var writeselector = true;
    var els;
    for (var i = 0; i < rules.length; i++) {
      var s1 = rules[i].selectorText, s2 = s1+'_Ki'+this.count;
      els = svg.querySelectorAll(s1);
      if (svg.classList) {
        for (var k = 0; k < els.length; k++) {
          els[k].classList.replace(s1.substring(1), s2.substring(1))
        }
      } else {
        // IE9-11 and other old browsers without classList
        var re = new RegExp('( |^)'+s1.substring(1)+'( |$)');
        for (var k = 0; k < els.length; k++) {
          els[k].className.baseVal = els[k].className.baseVal.replace(re, '$1'+s2.substring(1)+'$2');
        }      
      }
      if (writeselector) {
        rules[i].selectorText = s2;
        if (rules[i].selectorText != s2) writeselector = false
      }
      // IE9-11 and some old browsers have read-only rule.selectorText
      if (!writeselector) {
        var newrule = rules[i].cssText.replace(s1+' ', s2+' ');
        sheet.deleteRule(i);
        sheet.insertRule(newrule, i)
      }
    }
    els = svg.querySelectorAll("[id]");
    for (var k = 0; k < els.length; k++) {
      els[k].id += '_Ki'+this.count;
    }
    els = svg.getElementsByTagName("use");
    for (var k = 0; k < els.length; k++) {
      els[k].setAttribute('xlink:href', 
        els[k].getAttribute('xlink:href') + '_Ki'+this.count);
    }
    this.count++;
  },
  
  
  // Show/hide the text search form
  findbox: function(wrap, show) {
    var find = wrap.lastChild;
    find.style.display = show ? 'block' : '';
    if (show) find.firstChild.focus();
    wrap._finds = '';
  },
  // Find component reference/value
  // iterate through multiple matches with 'find_match'
  findtext: function(e) {
    var s = e.target.elements.q.value.toUpperCase();
    ...
    if (s != '') {
      ...
      var els = wrap.firstChild.querySelectorAll('text');
      var skip = wrap._nmatch;
      // Look for exact matches first, then substring matches
      for (var exact = 1; exact >= 0; exact--) {
        for (var k = 0; k < els.length; k++) {
          var su = els[k].textContent.toUpperCase();
          if (exact && (s != su || skip-- > 0)) continue;
          if (!exact && (su == s || su.indexOf(s) != 0 || skip-- > 0)) continue;
          wrap._findx = els[k].getAttribute('x');
          wrap._findy = els[k].getAttribute('y');
          wrap._panzoom.setCenter(wrap._findx, wrap._findy);
          wrap._nmatch++;
          if (exact) return false;
            else break;
        }
      }
      // Repeat final match? in case user has scrolled or zoomed
      if (wrap._nmatch && !skip) {
        wrap._panzoom.setCenter(wrap._findx, wrap._findy);
      }
    } else wrap.focus(); // reassign input focus will close the find box
    return false
  },
  
  
  // KiCad-compatible keyboard shortcuts
  keydown: function(e) {
    var f = e.currentTarget._panzoom;
    var k = e.key; // no e.code in IE
    
    // Transform the point from page co-ordinate system to SVG co-ordinate system
    function coordinateTransform(svgRoot, point) {
    ...
    if ((k == 'F1' || k == 'F2') && !e.shiftKey && !e.ctrlKey) { // with or without Alt+
      if (k == 'F1') f.zoomIn()
       else f.zoomOut();
      e.preventDefault()
    }
    if ((k == 'Home' || k == 'F4') && !e.shiftKey && !e.ctrlKey && !e.altKey) {
      if (k == 'F4') {
        var svg = e.currentTarget.firstChild;
        var pos = coordinateTransform(svg, e.currentTarget._mousepos);
        f.setCenter(pos.x, pos.y);
      // BUG: SVGPanZoom doesn't redraw correctly after setting intial viewbox?
      } else f.reset();
      e.preventDefault()
    }
    if (k.substr(0,5) == 'Arrow') k = k.substr(5); // IE doesn't use "Arrow" prefix
    if (['Left','Right','Up','Down'].indexOf(k) != -1 &&
      e.shiftKey && !e.ctrlKey) {
      var vb = f.getViewBox();
      // KiCad pans 1/20th of current viewable area per keypress
      if (k == 'Left') f.panLeft(vb.width/20)
        else if (k == 'Right') f.panRight(vb.width/20)
        else if (k == 'Up') f.panUp(vb.height/20)
        else f.panDown(vb.height/20);
      e.preventDefault();
    }
    // Search box open/close
    if ((k == 'f' || k == 'F') && e.ctrlKey) {
      KiCadWebView.findbox(e.currentTarget, true);
      e.preventDefault()
    }
    if ((k == 'Escape' || k == 'Esc') && e.target.name == 'q') { // IE uses "Esc"
      var wrap = e.target.parentElement.parentElement;
      e.target.value = '';
      wrap.focus();
      e.preventDefault()
    }
  ...
  // Display compressed SVG of KiCad PCB with layers UI, pan+zoom
  // svg = DOM element of <svg> tag, or element id (string)
  // mylayers = optional list of custom layer:colour mappings (object)
  // aspect = optional aspect ratio of DIV (number)
  pcb: function(svg, mylayers, aspect) {
    var layers = [];
    var flipped = false;
    
    // Set SVG layer 'ci' on/off
    function setlayer(ci, show){
      var els = null;
      ci = parseInt(ci);
      for (var i = 0; i < layers[ci].length; i++) {
        var j = layers[ci][i];
        if (typeof j === 'string') {
          var toggle = svg.querySelectorAll(j);
    ...
    // Toggle SVG layer (colour) based on checkbox element 'e'
    function togglelayer(e){
      if (e.target.nodeName != 'INPUT') return;
      setlayer(e.target.value, e.target.checked);
    }
    ...
    function pcb_keydown(e) {
      ...
      var k = e.key; // no e.code in IE
      // Show only F.* layers, or B.* layers
      if (k == 'PageUp' || k == 'PageDown') {
        var els = ui.querySelectorAll('INPUT');
        var front = (k == 'PageUp');
        for (var k = 0; k < els.length; k++) {
          var s = els[k].nextSibling.textContent;
          if (s.substr(0,2) == "F.") {
            els[k].checked = front;
            setlayer(els[k].value, front);
      ...
      if (k == 'f' || k == 'F') {
        flipped = !flipped;
        svg.style.transform = flipped ? 'ScaleX(-1)' : '';
        e.preventDefault()
      }
    }
    
    // Convert colour strings of the form "rgb(132, 132, 0)" => "#848400"
    ...
    for (var i = 0; i < rules.length; i++) {
      var c, ci;
      if (rules[i].style.fill && (c = rgb2Hex(rules[i].style.fill))) {
        if ((ci = cols.indexOf(c)) == -1) {
          cols.push(c);
          ci = cols.length-1;
          layers.push([rules[i].selectorText]);
        } else {
          layers[ci].push(rules[i].selectorText);
        }
      }
    };
  ...
  // Display compressed SVG of KiCad schematic with pan+zoom
  // svg = DOM element of <svg> tag, or element id (string)
  // aspect = optional aspect ratio of DIV (number)
  schematic: function(svg, aspect) {
    if (typeof svg === 'string') svg = document.getElementById(svg);
...
var SVGPanZoom = function () {
    function SVGPanZoom(svg, options) {
        ...
        if (!(svg instanceof SVGElement)) {
            throw new Error('Invalid Parameters. First parameter to SVGPanZoom should be an svg element');
        }
